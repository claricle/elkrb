...
VertiFlex:: column-grid arrangement that ignores edges
...
`layout_options`:: Hash of ELK option ids to values
...
`layout_options` is an open Hash on every model (graph, node, edge, port,
label). Keys are ELK option ids as strings; values are whatever that option
takes. There is no fixed attribute list -- any id ELK understands can be set.

[source,ruby]
----
graph.layout_options = {
  "elk.algorithm" => "layered",              # layered, force, stress, ...
  "elk.direction" => "DOWN",                 # DOWN, UP, LEFT, RIGHT
  "elk.spacing.nodeNode" => 20.0
}
----

Symbol keys are accepted and stringified on assignment, so
`{ "elk.algorithm" => "layered" }` and `{ :"elk.algorithm" => "layered" }`
are equivalent. Mutating the map in place after assignment does not
stringify, so use string keys there.

NOTE: `text` and `elements` are reserved by the serialization layer and
cannot be used as option keys.
